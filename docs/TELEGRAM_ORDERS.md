...
## Крок 3 — токен бота (НЕ в GitHub)

### Спосіб А — лист у таблиці (простіше)

1. Вставте оновлений `scripts/google-apps-script.gs` і **Розгорніть → Нова версія**.
2. Відкрийте в браузері (ваш URL):

   `.../exec?action=initTelegram`

3. У Google Таблиці з’явиться лист **`telegram`**.
4. У клітинку **B1** вставте токен від [@BotFather](https://t.me/BotFather) (`/mybots` → API Token).
5. **B2** вже має бути `-1003933471474` (група «Заказы»).

Збережіть таблицю — **нове розгортання не потрібне**.

### Спосіб Б — Script Properties

**Проєкт → Налаштування → Властивості скрипта:**
